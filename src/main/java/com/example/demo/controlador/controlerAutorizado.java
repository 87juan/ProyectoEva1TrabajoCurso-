...
public class controlerAutorizado {
	...
	public void guardarAutorizado(@Valid @RequestBody Autorizado newAutorizado) {
		...
		System.out.println("Error: dirección no existente");
		return;
	}
	...
	@DeleteMapping
	public void eliminarAutorizado(@Valid @RequestHeader Autorizado eliminaraAutorizado) {
		for (Integer i=0; i < this.autorizados.size() ; i++) {
			if(this.autorizados.get(i)==eliminaraAutorizado) {
				this.autorizados.remove(i);
				return;
			}
